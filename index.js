...
//   const apiKey = "9M5NGRDT68BAUQI5IRAYAZ4PDBV72YRZ3X";
//   const kyberAddress = '0xdd974d5c2e2928dea5f71b9825b8b646686bd200';
  
//   const abiResponse = await fetch(`https://api.etherscan.io/api?module=contract&action=getabi&address=${kyberAddress}&apikey=${apiKey}`);
//   const abiData = await abiResponse.json();
//   const abi = JSON.parse(abiData.result);
  
//   const priceResponse = await fetch(`https://api.etherscan.io/api?module=stats&action=tokensupply&contractaddress=${kyberAddress}&apikey=${apiKey}`);
//   const priceData = await priceResponse.json();
//   const totalSupply = parseInt(priceData.result) / Math.pow(10, 18);
  
//   const pricePerToken = totalSupply > 0 ? (1 / totalSupply).toFixed(6) : 0;
  
//   console.log(`Kyber Network ABI: ${JSON.stringify(abi)}`);
//   console.log(`Kyber Network Token Price: ${pricePerToken}`);
// }



    // // Dont forget to put in exhange ABI functions above for contract checking
    // const daiAmount = await exchangeContract.methods.getEthTokenInputPrice(ETH_AMOUNT).call()
